    ...
    {mode === 'login' ? (
       ...
       <div className="space-y-5">
        <div>
         ...
         <div className="relative">
          ...
          <input type="email" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-100 dark:bg-slate-800 shadow-sm border border-slate-300 dark:border-slate-700 rounded-2xl text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/30 focus:outline-none focus:border-green-500/50 focus:ring-1 focus:ring-green-500/50 transition-all" placeholder="email@contoh.com" />
         </div>
        </div>
        <div>
     ...
     <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6 flex flex-col">
       <div className="bg-slate-100 dark:bg-slate-800 shadow-sm border border-slate-300 dark:border-slate-700 rounded-3xl p-8 flex-1 flex flex-col">
         <h3 className="text-xl font-bold mb-6">Perekaman 3 Sudut</h3>
         
         <div className="space-y-4 flex-1">
          {(['depan', 'kiri', 'kanan'] as const).map((angle) => (
            <div key={angle} className={`border p-4 rounded-2xl flex items-center gap-4 transition-all ${activeAngle === angle ? 'border-green-500/50 bg-green-500/5' : 'border-slate-300 dark:border-slate-700 bg-white/5'}`}>
             <div className="w-16 h-16 rounded-xl border border-slate-300 dark:border-slate-700 bg-white/80 dark:bg-black/50 overflow-hidden flex items-center justify-center shrink-0">
