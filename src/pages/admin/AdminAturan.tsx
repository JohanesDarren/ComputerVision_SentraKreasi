    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full p-4 md:p-8 text-slate-900 dark:text-white relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
           ...
           <div className="col-span-full py-20 text-center text-slate-700 dark:text-white/50 bg-white/95 dark:bg-white/5 shadow-xl dark:shadow-none border border-slate-900/10 dark:border-white/10 rounded-3xl backdrop-blur-md">
              <p className="text-sm font-medium">Belum ada aturan yang dibuat.</p>
           </div>
        ) : (
          aturan.map((item) => (
            <div key={item.id} className="bg-white/95 dark:bg-white/5 shadow-xl dark:shadow-none border border-slate-900/10 dark:border-white/10 p-6 rounded-3xl backdrop-blur-xl flex flex-col relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-green-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
              
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="flex items-center gap-3">
                   <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${item.is_active ? 'bg-green-500 text-white dark:text-black dark:text-black border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.3)]' : 'bg-white/95 dark:bg-white/5 shadow-xl dark:shadow-none text-slate-700 dark:text-white/40 border-white/10'}`}>
              ...
              <div className="space-y-4 flex-1 relative z-10">
                 <div className="flex justify-between border-b border-slate-900/10 dark:border-white/10 pb-2">
              ...
              <div className="flex gap-2 mt-8 pt-4 border-t border-slate-900/10 dark:border-white/10 relative z-10">
                 <button 
                   ...
                   className={`flex-1 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-all border ${
                     item.is_active 
                     ? 'bg-white/95 dark:bg-white/5 shadow-xl dark:shadow-none border-slate-900/10 dark:border-white/10 text-slate-700 dark:text-white/60 hover:text-white' 
