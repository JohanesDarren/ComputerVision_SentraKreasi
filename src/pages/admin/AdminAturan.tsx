...
export default function AdminAturan() {
 ...
 return (
  <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-slate-900 dark:text-white relative">
   ...
   <div className="bg-slate-100 dark:bg-slate-800 shadow-sm border border-slate-300 dark:border-slate-700 p-5 rounded-2xl w-full relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div className="absolute top-0 right-0 w-32 h-32 bg-green-400/30 dark:bg-green-500/20 blur-3xl rounded-full"></div>
    <div className="relative z-10">
     <h1 className="text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-white/60">Aturan Presensi</h1>
     <p className="text-xs font-medium text-slate-700 dark:text-white/50 mt-1">Konfigurasi jam kerja dan tanggal libur nasional.</p>
    </div>
   </div>

   {isLoading ? (
    <div className="py-12 flex justify-center items-center text-slate-700 dark:text-white/50">
     <Loader2 className="w-6 h-6 animate-spin text-green-400" />
    </div>
   ) : aturan && (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
     {/* Kartu Jam Kerja */}
     <div className="bg-slate-100 dark:bg-slate-800 shadow-sm border border-slate-300 dark:border-slate-700 p-5 rounded-2xl relative overflow-hidden flex flex-col">
      <div className="flex justify-between items-center mb-4">
       <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-green-400/20 dark:bg-green-500/10 text-green-500 border border-green-500/20 shadow-[0_0_10px_rgba(34,197,94,0.1)]">
         <Clock className="w-5 h-5" />
        </div>
        <h3 className="text-sm font-bold text-slate-900 dark:text-white">Jam Kerja</h3>
       </div>
       <button onClick={handleEditAturan} className="p-1.5 bg-slate-200 dark:bg-slate-700 rounded-full hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
        <Edit2 className="w-3.5 h-3.5 text-slate-700 dark:text-white/70" />
       </button>
      </div>
      <div className="space-y-3 flex-1">
       <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-700 pb-2">
        ...
        <span className="text-sm font-bold text-slate-900 dark:text-white">{aturan.jam_masuk} WIB</span>
       </div>
       <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-700 pb-2">
        <span className="text-xs font-medium text-slate-700 dark:text-white/50 uppercase tracking-widest">Jam Pulang</span>
        <span className="text-sm font-bold text-slate-900 dark:text-white">{aturan.jam_keluar} WIB</span>
       </div>
       <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-700 pb-2">
        <span className="text-xs font-medium text-slate-700 dark:text-white/50 uppercase tracking-widest">Batas Pulang</span>
        <span className="text-sm font-bold text-slate-900 dark:text-white">{aturan.jam_batas_pulang} WIB</span>
       </div>
       <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-700 pb-2">
        <span className="text-xs font-medium text-slate-700 dark:text-white/50 uppercase tracking-widest">Toleransi</span>
        <span className="text-sm font-bold text-slate-900 dark:text-white">{aturan.toleransi_menit} Menit</span>
       </div>
      </div>
     </div>
     ...
     <div className="bg-slate-100 dark:bg-slate-800 shadow-sm border border-slate-300 dark:border-slate-700 p-5 rounded-2xl relative overflow-hidden flex flex-col">
      <div className="flex justify-between items-center mb-4">
       <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-blue-400/20 dark:bg-blue-500/10 text-blue-500 border border-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.1)]">
         <Calendar className="w-5 h-5" />
        </div>
        <h3 className="text-sm font-bold text-slate-900 dark:text-white">Hari Libur</h3>
       </div>
       <button onClick={handleAddLibur} className="p-1.5 bg-slate-200 dark:bg-slate-700 rounded-full hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
        <Plus className="w-3.5 h-3.5 text-slate-700 dark:text-white/70" />
       </button>
      </div>
      <div className="space-y-2 flex-1 overflow-y-auto max-h-[160px] pr-2 custom-scrollbar">
       {!aturan.hari_libur || aturan.hari_libur.length === 0 ? (
        <div className="text-center py-6 text-xs font-medium text-slate-700 dark:text-white/40">
        ...
        aturan.hari_libur.map((item: any, i: number) => {
         ...
         return (
         <div key={item.tanggal || i} className="flex justify-between items-center p-2.5 bg-slate-200 dark:bg-slate-700/50 rounded-lg border border-slate-300 dark:border-slate-600">
          <div>
           <h4 className="text-xs font-bold text-slate-900 dark:text-white">{item.nama}</h4>
           <span className="text-[10px] font-semibold text-slate-700 dark:text-white/50 block mt-0.5">
            {!isNaN(validDate.getTime()) ? format(validDate, 'dd MMMM yyyy', {locale: localeID}) : 'Tanggal tidak valid'}
           </span>
          </div>
          <button type="button" onClick={() => removeHariLibur(item.tanggal)} className="text-red-400 hover:text-red-500 p-1.5 rounded-md hover:bg-red-500/10">
